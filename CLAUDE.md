...
**Parallel D.3 attempt (parked).** A separate D.3 effort started in the main repo before this branch was created. It was never deployed and is now superseded by what's live, except for one new feature: an owner-only "Accounts" management surface (`src/api/owner.js` — list users in your owned groups, change their email/password, delete with tombstoning). That work lives on the `wip/parallel-d3-with-accounts` branch on origin. When picking up Accounts management as a follow-up, cherry-pick only `src/api/owner.js` from that branch and write a SuperDashboard "Accounts" tab to call it; everything else on that branch is parallel/superseded.
