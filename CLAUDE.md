...
**D.3 follow-ups (soak-period stabilization).** Round of fixes shipped during the 3-day soak before D.3 finalization. The SuperDashboard now has three tabs: **Groups**, **Accounts**, **Profile**.

- **Profile screen.** Self-edit surface for cloud accounts. `PATCH /api/me` ([src/api/auth.js](src/api/auth.js)) accepts optional `displayName`, `email`, `password` — the last requires `currentPassword` for verification (same PBKDF2 path as `/api/auth/password`). `ProfilePage` ([ShiftApp.v3.jsx](ShiftApp.v3.jsx)) renders cloud info (display name / email editable, username read-only, collapsible Change-password form) plus a Group-profile section for in-group views (name/role/points read-only, shift targets editable). Reachable from the in-group sidebar user-footer (whole row is the click target; mobile uses the avatar in the topbar) or from the new SuperDashboard "Profile" tab. The `superContext` prop on `ProfilePage` gates the local section off for owners (who have no local user record). Component-local helpers `seedProfileForm` / `openProfilePage` / `saveProfileInfo` / `saveProfilePassword` keep separate `infoBusy` and `pwBusy` so the two sections don't block each other.

- **Group-code drift heal.** `PATCH /api/groups/:gid/codes` ([src/api/groups.js](src/api/groups.js)) — owner-only, uniqueness-checked, idempotent on no-op. `rollGroupCodes` now pushes the new codes to cloud BEFORE persisting locally, so the dashboard never drifts ahead of cloud again. `/api/me` and `/api/auth/signup` responses now include `groupCode`/`adminCode` per membership. New one-shot `useEffect` (gated by `codesReconciledRef`) pulls cloud codes → local on the first SuperDashboard mount when they differ — heals the legacy case where an owner rolled codes locally before the PATCH endpoint existed.

- **Owner-group auto-claim.** Fresh owner sign-in (incognito, new device) used to land on an empty SuperDashboard because the cloud bridge skipped past the auth-screen "Restore" cards. New `useEffect` (gated by `ownerGroupsClaimedRef`) walks `cloudUser.memberships` filtered to `role === "owner"`, pulls the latest snapshot for each missing `cloudGroupId`, and creates the local mirror automatically with one batched toast (`✅ Restored N groups from cloud`).

- **Snapshot history + restore.** R2-backed recovery flow for owners. `GET /api/owner/snapshots/:gid/history` ([src/api/snapshots.js](src/api/snapshots.js)) lists R2 objects newest-first with summary stats — user count / prefs count / shift-day count / top-option-day count — for the 10 most recent (lazy-loads payloads, capped to `limit=30` default / `100` max). `POST /api/owner/snapshots/:gid/restore { key }` validates the `snapshots/<gid>/` prefix, copies the named R2 object into the D1 `snapshots` row as the new latest, AND writes a fresh history entry tagged `restoredFrom` in customMetadata so the audit trail stays intact. Frontend `SnapshotHistoryModal` on each cloud-mirrored group card lists rows with timestamps + summary + a Restore button (disabled for the row matching current `serverTs`/`clientTs`). After a successful restore the response payload is also `applySnapshot`'d locally so the UI refreshes immediately.

- **Sync from cloud.** Companion green button on each cloud-mirrored group card (`syncGroupFromCloud`). Pulls the current `/api/snapshots/:gid/latest` and `applySnapshot`'s it locally — the idempotent recovery path when local is corrupted but cloud is fine. Bypasses the existing sync-banner timestamp comparison so the owner can trigger it on demand.

- **Ghost-account lookup + orphan delete.** `GET /api/owner/lookup?email=…` ([src/api/owner.js](src/api/owner.js)) — gated on `canCreateGroups`. Returns the user plus ALL their memberships with a `callerOwns` flag per group, surfacing cloud accounts that don't appear in `/api/owner/users` (members of groups the caller doesn't own, or with no memberships at all). `deleteOwnerUser` got a second auth path: if the caller has `canCreateGroups` AND the target has zero memberships, deletion is allowed (orphan-cleanup case for accounts that signed up with the bootstrap owner code but never created/joined a group). UI: "Look up by email" panel on the Accounts tab.

- **Local-only dedupe fix.** ⚠ Bug aftermath worth documenting: the Accounts list's "Local only" badge originally only matched local users to cloud users by `cloudUserId` or `email`, missing the third path used by D.2-migrated test users (cloud `memberships.local_uid` ↔ local `id`). Result: every D.2-migrated user falsely appeared as Local-only with a Remove button, and clicking Remove silently wiped them from local `users[]` (preferences / topOptions / shifts intact — just the user roster). The fix in `loadAccounts` adds a `cloudLocalUidPairs` set built from each cloud user's `memberships`. An actual user hit this; existing corruption was recoverable via "Sync from cloud". If you see a similar local-vs-cloud roster mismatch report, this dedupe is the first place to look.
...
- ✅ **D.3 follow-ups:** Profile screen (cloud self-edit), R2 snapshot history + restore, Sync-from-cloud button, group-code drift heal, owner-group auto-claim on fresh devices, ghost-account lookup + orphan-user cleanup
