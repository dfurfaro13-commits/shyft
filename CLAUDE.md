...
- ✅ **CSV schedule export (locked blocks)** — `exportScheduleCsv(block)` helper (component-local, near `getBlockReport`) emits `Date,Day,<slot.name>,…` rows for the block's date range, using admin-defined slot names as column headers and empty cells for open slots. RFC 4180 quoting + UTF-8 BOM so Excel auto-detects encoding. Filename `shift-schedule-<slug>.csv`. Surfaced as a `⬇ .csv` button gated on `phase === PHASE.LOCKED`: in the SchedulePage toolbar (providers) and in the ShiftsPage header (admin Calendar).
